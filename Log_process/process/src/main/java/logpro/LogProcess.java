...
public class LogProcess 
{
    public static void main( String[] args )
    {
    	...
    	else
    	{
    		...
    		BatchExecute batch = new BatchExecute(line.getOptionValue("input"), line.getOptionValue("output"));
    		batch.run();
    	}
    ...
    private static CommandLine setupOptions(String[] args)
    {
    	...
    	Option input = Option.builder("input").desc( "input file/directory"  ).hasArg(true).argName( "input" ).required().build();
    	options.addOption(input);

    	Option output = Option.builder("output").desc( "output file/directory"  ).hasArg(true).argName( "output" ).required().build();
